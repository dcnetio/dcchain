...
use pallet_dc_node::{SpaceSize, LoginTimes};
use frame_support::{
	dispatch::Codec,
	pallet_prelude::*,
	traits::{
...
use sp_std::{convert::From, prelude::*, collections::{btree_map::BTreeMap},};
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		/// Something that provides the dc functionality.
		#[cfg(feature = "dc_lib")]
		type DcProvider: pallet_dc_node::DcProvider<
			AccountId = Self::AccountId,
	...
	/// List of eras for which the storage nodes have claimed rewards.
	#[pallet::storage]
	pub type StorageClaimedRewards<T: Config> = StorageMap<_, Twox64Concat, T::AccountId, Vec<EraIndex>>;

	/// The total storage era payout for the last `HISTORY_DEPTH` eras.
	///
	/// Eras that haven't finished yet or has been removed doesn't have reward.
	#[pallet::storage]
	#[pallet::getter(fn eras_storage_reward)]
	pub type ErasStorageReward<T: Config> = StorageMap<_, Twox64Concat, EraIndex, BalanceOf<T>>;

	/// The percent of storage rewards in the total issuance
	#[pallet::storage]
	#[pallet::getter(fn storage_reward_percent)]
	pub type StorageRewardPercent<T: Config> = StorageValue<_, u32, ValueQuery>;

	/// The space size of the storage nodes
	#[pallet::storage]
	#[pallet::getter(fn storage_node_space)]
	pub type StorageNodeSpace<T: Config> = StorageMap<
		_,
		Twox64Concat,
		EraIndex,
		(SpaceSize, BTreeMap<T::AccountId, SpaceSize>),
		ValueQuery,
	>;

	/// List of eras for which the apps have claimed rewards.
	#[pallet::storage]
	pub type AppClaimedRewards<T: Config> = StorageMap<_, Twox64Concat, T::AccountId, Vec<EraIndex>>;

	/// The total app era payout for the last `HISTORY_DEPTH` eras.
	///
	/// Eras that haven't finished yet or has been removed doesn't have reward.
	#[pallet::storage]
	#[pallet::getter(fn eras_app_reward)]
	pub type ErasAppReward<T: Config> = StorageMap<_, Twox64Concat, EraIndex, BalanceOf<T>>;

	/// The login times of the apps
	#[pallet::storage]
	#[pallet::getter(fn app_login_times)]
	pub type AppLoginTimes<T: Config> = StorageMap<
		_,
		Twox64Concat,
		EraIndex,
		(LoginTimes, BTreeMap<T::AccountId, LoginTimes>),
		ValueQuery,
	>;
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
		fn build(&self) {
			...
			StorageRewardPercent::<T>::put(60u32);
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn unbond(
			...
			value = value.min(Self::can_be_unbound_amount(&ledger.stash, value));
		...
		/// Set the percent of storage rewards in the total issuance.
		#[pallet::call_index(26)]
		#[pallet::weight(T::DbWeight::get().reads_writes(0,1))]
		pub fn set_storage_reward_percent(
			origin: OriginFor<T>,
			percent: u32, 
		) -> DispatchResult {
			ensure_root(origin)?;
			ensure!(percent < 100, Error::<T>::InvalidPercent,);
			<StorageRewardPercent::<T>>::put(percent);
			Ok(())
		}

		#[pallet::call_index(27)]
		#[pallet::weight(T::WeightInfo::payout_storage())]
		pub fn payout_storage(
			origin: OriginFor<T>,
			reward_stash: T::AccountId,
			era: EraIndex,
		) -> DispatchResultWithPostInfo {
			ensure_signed(origin)?;
			Self::do_payout_storage(reward_stash, era)
		}

		#[pallet::call_index(28)]
		#[pallet::weight(T::WeightInfo::payout_app())]
		pub fn payout_app(
			origin: OriginFor<T>,
			reward_stash: T::AccountId,
			era: EraIndex,
			...
			ensure_signed(origin)?;
			Self::do_payout_app(reward_stash, era)
		}
	}
}
